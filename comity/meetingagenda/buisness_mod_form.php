...
/**
 * The form for the minutes tab for the Agenda/Meeting Extension to Committee Module.
 *
 *
...
class mod_buisness_mod_form extends moodleform {
     ...
     * @param int $comity_id Committee ID
        ...
        if ($commityRecords) {//If any committee members present
            ...
            foreach ($commityRecords as $member) {
                ...
                $participant = array();
                    ...
                    $participant[] = & $mform->createElement('static',"test","","<tr><td>");
...
$attendance = $DB->get_record('comity_agenda_attendance', array('comity_agenda' => $agenda_id,'comity_members'=>$member->id));
...
$moodle_members = $DB->get_records_sql($sql, array($agenda_id), $limitfrom=0, $limitnum=0);
            ...
            $participant[] = & $mform->createElement('image',"remove_moodle_user[$index]", "$CFG->wwwroot/mod/comity/pix/delete.gif");
...
$mform->addElement('html', $test);

$mform->registerNoSubmitButton('new_moodle_member');

$mform->addElement('submit', 'new_moodle_member', get_string('add_moodle_user','comity'));
...
$guests = $DB->get_records_sql($sql, array($agenda_id), $limitfrom=0, $limitnum=0);
            ...
            $participant = array();
                ...
                $participant[] = & $mform->createElement('image',"remove_guest[$index]", "$CFG->wwwroot/mod/comity/pix/delete.gif");
...
//------------------------------------------------------------------------------
  ...
  $sql = "SELECT DISTINCT cag.firstname,cag.lastname FROM {comity_agenda_guests} cag,{comity_events} ce,{comity_agenda} ca WHERE cag.moodleid IS NULL " .
      "AND cag.comity_agenda = ca.id AND ".
    "ca.comity_id = ce.comity_id AND ".
    "ca.comity_events_id = ce.id AND ".
    "ce.comity_id = ?"
    ;

  //Get all previous guests for this committee
 $prev_guests = $DB->get_records_sql($sql, array($comity_id), $limitfrom=0, $limitnum=0);
...
$mform->addGroup($add_prev_guest, "prev_guest_group", get_string('add_previous_guest', 'comity') , array(' '), false);
$mform->registerNoSubmitButton('add_prev_guest');

}

$mform->addElement('static', "", '', "");
$mform->addElement('static', "", '', get_string('add_new_guest', 'comity'));
$mform->addElement('text', 'guest_firstname', get_string('add_new_guest_first', 'comity'),"");
$mform->addElement('text', 'guest_lastname', get_string('add_new_guest_last', 'comity'),"");
$mform->addElement('submit', 'add_new_guest',get_string('add_new_guest', 'comity'));
...
$mform->addElement('select', "topic_status[$index]", get_string('topic_status', 'comity'), $topic_statuses, $attributes=null);
$mform->addElement('hidden', "topic_ids[$index]", $topic->id);
//$mform->addElement('text', "follow_up[$index]", get_string('topic_followup', 'comity'), array('size'=>'50'));

//-------FILE MANAGER ----------------------------------------------------------
$mform->addElement('filemanager', "attachments[$index]", get_string('attachments', 'comity'), null,array('subdirs' => 0, 'maxbytes' => 0, 'maxfiles' => 10, 'accepted_types' => array('*')) );
...
if($motions){
  ...
  $mform->addElement('static', "", "", "<h6>".get_string('motion', 'comity')." $index.$sub_index:</h6>");
   //$mform->addElement('text', "proposition[$index][$sub_index]", get_string('motion_proposal', 'comity'), array('size'=>'50'));
...
$mform->addGroup($member_support, "member_support[$index][$sub_index]", get_string('motion_support', 'comity') , array(' '), false);
   
$votes = array();
    $votes[] =$mform->createElement('static', "", "", "Aye: ");
    $votes[] =$mform->createElement('text', "aye[$index][$sub_index]", '', array('size'=>'1 em','maxlength'=>"3"));
    $votes[] =$mform->createElement('static', "", "", "Nay: ");
    $votes[] =$mform->createElement('text', "nay[$index][$sub_index]", '', array('size'=>'1 em','maxlength'=>"3"));
    $votes[] =$mform->createElement('static', "", "", "Abs: ");
    $votes[] =$mform->createElement('text', "abs[$index][$sub_index]", '', array('size'=>'1 em','maxlength'=>"3"));
    $votes[] = $mform->createElement('checkbox', "unanimous[$index][$sub_index]", '', " ".get_string('unanimous','comity'));
...
$supporting_choices['-1']=get_string('supportedby', 'comity');
//$mform->addElement('static', "", "", "----------------------------------");
$mform->addElement('static', "", "", "<h6>".get_string('new_motion', 'comity').":</h6>");
//$mform->addElement('text', "proposition_new[$index]", get_string('motion_proposal', 'comity'), array('size'=>'50'));
...
$mform->addGroup($member_support, "member_support_new[$index]", get_string('motion_support', 'comity') , array(' '), false);


$votes = array();
    $votes[] =$mform->createElement('static', "", "", "Aye: ");
    $votes[] =$mform->createElement('text', "aye_new[$index]", '', array('size'=>'1 em','maxlength'=>"3"));
    $votes[] =$mform->createElement('static', "", "", "Nay: ");
    $votes[] =$mform->createElement('text', "nay_new[$index]", '', array('size'=>'1 em','maxlength'=>"3"));
    $votes[] =$mform->createElement('static', "", "", "Abs: ");
    $votes[] =$mform->createElement('text', "abs_new[$index]", '', array('size'=>'1 em','maxlength'=>"3"));
$votes[] = $mform->createElement('checkbox', "unanimous_new[$index]", '', " ".get_string('unanimous','comity'));

    $mform->addGroup($votes, "motion_votes_new[$index]", get_string('motion_votes', 'comity') , array(' '), false);

    $results = array();
    $results[] = $mform->createElement('select', "motion_result_new[$index]", '', $motion_result, $attributes=null);
    $mform->addGroup($results, "result_new[$index]", get_string('motion_outcome', 'comity') , array(' '), false);


$mform->addGroupRule("motion_votes_new[$index]",
 array("aye_new[$index]" => array(array(get_string('numeric_only','comity'), 'numeric', null, 'client', false, true)),
       "nay_new[$index]" => array(array(get_string('numeric_only','comity'), 'numeric', null, 'client', false, true)),
       "abs_new[$index]" => array(array(get_string('numeric_only','comity'), 'numeric', null, 'client', false, true))
     ));

$mform->addElement('submit', "add_motion[$index]", get_string('add_update_motions','comity'));
